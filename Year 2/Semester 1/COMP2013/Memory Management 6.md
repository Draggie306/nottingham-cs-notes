...
### The vicious cycle
CPU usage becomes low, so the scheduler increases the amount of multiprogramming, allocating frames to new processes, taking away from existing processes, queuing IO requests as a result of page faults. This adds more processes to memory to increase CPU usage as the usage drops further…

Counter-intuitively, the memory allocated per process is too small, so the working set for each process cannot be kept in process. Therefore we should terminate process to increase memory per process, to reduce page fault rates and improve CPU usage. 
