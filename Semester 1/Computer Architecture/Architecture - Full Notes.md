...
### Pipeline hazards 
A **pipeline bubble** is a stage when some parts of the CPU are not doing anything because a hazard forced the delay of an earlier stage, slowing execution times. It can (amongst other things) be caused by a structural hazard: when one execute instruction requires to access memory, so will the fetch stage in the pipeline - this can't happen. 
...
A control hazard introduces a bubble that is 1 stage less long than the pipeline - 3 cycles for FDE, so the stall is 2 cycles. This happens when branching.
